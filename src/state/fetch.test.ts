...
describe('fetch.ts', () => {
  ...
  describe('ResourceInstance', () => {
    it('creates a ResourceInstance correctly', () => {
      ...
      const instance = new ResourceInstance('123', 'test/123', { name: 'Test' }, schema);
  ...
  describe('List function', () => {
    ...
    it('handles HTTP error status', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('List failed with status 404');
    ...
    it('handles HTTP error status with error field in response', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('List failed: Resource not found');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('handles HTTP error status with message field in response', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('List failed: Invalid request parameters');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('handles API error response with errors field', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('API Error: Resource not found, Invalid parameters');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('handles API error response with code and message', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('400: Bad Request');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('handles network errors', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('Failed to list resources: Network error');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Get function', () => {
    ...
    it('handles HTTP error status', async () => {
      ...
      await expect(Get('http://test.com/api/1', schema)).rejects.toThrow('Get failed with status 404');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Create function', () => {
    it('successfully creates a resource', async () => {
      ...
      await expect(Create('http://test.com/api', createData)).resolves.toBeUndefined();
    ...
    it('handles HTTP error status', async () => {
      ...
      await expect(Create('http://test.com/api', {})).rejects.toThrow('Create failed with status 400');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('handles API error in response body', async () => {
      ...
      await expect(Create('http://test.com/api', {})).rejects.toThrow('API Error: Validation failed');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Delete function', () => {
    ...
    it('handles HTTP error status', async () => {
      ...
      await expect(Delete('http://test.com/api/1')).rejects.toThrow('Delete failed with status 404');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Patch function', () => {
    it('successfully patches a resource', async () => {
      ...
      await expect(Patch('http://test.com/api/1', patchData)).resolves.toBeUndefined();
    ...
    it('handles HTTP error status', async () => {
      ...
      await expect(Patch('http://test.com/api/1', {})).rejects.toThrow('Patch failed with status 422');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Error detection', () => {
    it('detects errors field as string', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('API Error: Single error message');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('detects errors field as array', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('API Error: Error 1, Error 2, Error 3');
      expect(toast).toHaveBeenCalledWith({
    ...
    it('detects code and message only response', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('INVALID_REQUEST: The request is invalid');
      expect(toast).toHaveBeenCalledWith({
  ...
  describe('Header parsing', () => {
    it('parses headers correctly', async () => {
      ...
      const schema = createMockResourceSchema();
      await List('http://test.com/api', schema, 'Authorization: Bearer token, Content-Type: application/json');
  ...
  describe('Edge cases', () => {
    ...
    it('re-throws API errors without double handling', async () => {
      ...
      await expect(List('http://test.com/api', schema)).rejects.toThrow('API Error: API Error');
