...
function substituteParamsWithKeys(pathname: string, params: Record<string, string | undefined>): string {
    let result = pathname;
    ...
    for(let idx = 0; idx < uniqueBreadcrumbs.length; idx++) {
        ...
        if (isLast) {
            ...
            breadcrumb_elements.push(
                <BreadcrumbItem key={`item-${elementIndex}`} className="hidden md:block">
        ...
        if (!isLast) {
            breadcrumb_elements.push(<BreadcrumbSeparator key={`separator-${elementIndex}`} className="hidden md:block" />);
            elementIndex++;
        }
    }
